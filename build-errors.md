...
- `arch/arm64/include/asm/proc-fns.h` - 删除冲突所在的 cpu_soft_restart 声明
...
2. 部分 `.c` 源文件（如 `xt_dscp.c`, `xt_rateest.c`, `xt_tcpmss.c`）在 Git 索引中内容重复，导致 Match 模块的代码实际上已经丢失。
...
3. **源文件修复**: 使用 `git update-index --cacheinfo` 恢复 Git 索引中丢失的 Match 模块源文件内容（`xt_dscp.c`, `xt_rateest.c`, `xt_tcpmss.c`），确保它们具有独立且正确的匹配逻辑。
4. **移除 Workaround**: 撤销之前在各 `.c` 模块中添加的所有临时结构体定义，恢复代码逻辑纯净。
...
## 2026-03-09 11:06 - Run 22836216634

**错误**: `签名 ZIP` 步骤失败，退出码 8。

**原因**: 
1. `ubuntu:20.04` 容器环境缺失 Java 运行时 (JRE/JDK)，导致 `java -jar zipsigner.jar` 无法执行。
2. `wget` 报错（退出码 8 可能暗示服务器错误或证书问题）。

**修复**: 
1. 在 `安装编译依赖` 步骤中添加 `openjdk-17-jdk-headless`。
2. 移除 `wget` 的 `-q` 参数并增加 `ls -l` 检查，以便排查文件是否存在。

**修改文件**:
- `.github/workflows/build-kernel.yml` - 添加 Java 依赖并优化签名步骤。
