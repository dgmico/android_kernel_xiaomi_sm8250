...
- **修复对策**: 
    1. 在 `magiskboot repack` 步骤中增加 `dtbo` 的处理逻辑。
    2. 使用 `magiskboot patch` 明确针对内核进行 AVB 禁用处理（如果 `repack` 自动处理逻辑不生效）。
...
## [2026-03-13 09:30] S3 Rebuild 方案宣告失败与 AnyKernel3 回归
- **现象**: 即使使用了完整官方底包和极简重组策略，依然无法绕过 Bootloader 的拒收，且 PSTORE 为空，说明镜像物理结构（Header/Padding）不符合原厂严苛要求。
- **原因分析**: 服务端重组镜像（Repack）很难 100% 模拟手机本地的原始分区结构。AnyKernel3 在手机 Recovery 环境下直接对原始分区进行“原地替换”，是绕过签名校验差异的最稳妥办法。
- **修复对策 (v13 - Final Stand)**:
    1. **回归 AnyKernel3**: 在工作流中实时拉取最新的 AnyKernel3 框架。
    2. **集成 MagiskBoot v28**: 确保支持最新的安卓版本。
    3. **本地 Patch**: 让 AnyKernel3 在刷入时自动对内核进行 AVB 补丁。
    4. **保留调试**: 依然开启 PSTORE，以便万一崩溃时能看到日志。
- **验证**: 重新使用 AnyKernel 方式打包并观察开机情况。
